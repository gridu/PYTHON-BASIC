...
Implementation with Thread module, approximate time of execution: 22 seconds.
...
def get_company_info_tab_urls(soup: BeautifulSoup, specification: str) -> str:
        ...
        while True:
            ...
            profile.append(
                get_data_from_profile(
                    request_page(
                        get_company_info_tab_urls(comp_soup, 'profile'))))
            statistics.append(
                get_data_from_statistics(
                    request_page(get_company_info_tab_urls(comp_soup, 'statistics'))))
            holders.append(
                get_data_from_holders(
                    request_page(
                        get_company_info_tab_urls(comp_soup, 'holders'))))
...
def pprint(profile: pandas.DataFrame, statistics: pandas.DataFrame, holders: pandas.DataFrame) -> None:
    print(profile, statistics, holders, sep='\n')
